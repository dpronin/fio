...
static void gfio_text_op(struct fio_client *client, struct fio_net_cmd *cmd)
{
	...
	struct gfio_client *gc = static_cast<struct gfio_client *>(client->client_data);
...
static void gfio_disk_util_op(struct fio_client *client, struct fio_net_cmd *cmd)
{
	...
	gc->du = static_cast<decltype(gc->du)>(realloc(gc->du, (nr + 1) * sizeof(struct cmd_du_pdu)));
				 ...
				 struct group_run_stats *rs)
{
	...
	gc->results = static_cast<decltype(gc->results)>(realloc(gc->results, (nr + 1) * sizeof(struct end_results)));
					  ...
					  int num)
{
	...
	types = static_cast<GType*>(malloc(num * sizeof(GType)));
...
{
	struct graph *g = static_cast<struct graph*>(p);
	cairo_t *cr;
				       ...
				       gpointer data)
{
	...
	struct graph *g = static_cast<struct graph *>(data);
				  ...
				  struct thread_stat *ts, int ddir)
{
	...
	bw_palt = num2str(bw, ts->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(ts->unit_base));
