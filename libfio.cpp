...
{
	static_assert(TD_LAST == 12, "td runstate list");
...
int initialize_fio(char *envp[])
{
	 ...
	 * access (ARM).
	 */
	static_assert((offsetof(struct thread_data, ts) % sizeof(void *)) == 0, "ts");
	static_assert((offsetof(struct thread_stat, percentile_list) % 8) == 0, "stat percentile_list");
	static_assert((offsetof(struct thread_stat, total_run_time) % 8) == 0, "total_run_time");
	static_assert((offsetof(struct thread_stat, total_err_count) % 8) == 0, "total_err_count");
	static_assert((offsetof(struct thread_stat, latency_percentile) % 8) == 0, "stat latency_percentile");
	static_assert((offsetof(struct thread_data, ts.clat_stat) % 8) == 0, "ts.clat_stat");
	static_assert((offsetof(struct thread_options_pack, zipf_theta) % 8) == 0, "zipf_theta");
	static_assert((offsetof(struct thread_options_pack, pareto_h) % 8) == 0, "pareto_h");
	static_assert((offsetof(struct thread_options_pack, percentile_list) % 8) == 0, "percentile_list");
	static_assert((offsetof(struct thread_options_pack, latency_percentile) % 8) == 0, "latency_percentile");
	static_assert((offsetof(struct jobs_eta, m_rate) % 8) == 0, "m_rate");

	static_assert(__TD_F_LAST <= TD_ENG_FLAG_SHIFT, "TD_ENG_FLAG_SHIFT");
	static_assert(BSSPLIT_MAX <= ZONESPLIT_MAX, "bsssplit/zone max");
