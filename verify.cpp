...
{
	(void)cpy_pattern(td->o.buffer_pattern, td->o.buffer_pattern_bytes, static_cast<char*>(p), len);
}
				 ...
				 uint64_t seed, int use_seed)
{
	...
	fill_verify_pattern(td, io_u->buf, io_u->buflen, io_u, seed, use_seed);

	hdr_inc = get_hdr_inc(td, io_u);
	header_num = 0;
	for (char *p = static_cast<char*>(io_u->buf); p < static_cast<char*>(io_u->buf) + io_u->buflen; p += hdr_inc) {
		hdr = reinterpret_cast<decltype(hdr)>(p);
...
static void *hdr_priv(struct verify_header *hdr)
{
	...
	return static_cast<char*>(priv) + sizeof(struct verify_header);
}
...
static void __dump_verify_buffers(struct verify_header *hdr, struct vcont *vc)
{
	...
	dump_buf(static_cast<char*>(io_u->buf) + hdr_offset, hdr->len, io_u->verify_offset + hdr_offset,
	...
	dump_buf(static_cast<char*>(buf) + hdr_offset, hdr->len, io_u->verify_offset + hdr_offset,
...
{
	return static_cast<char*>(vc->io_u->buf) + vc->hdr_num * hdr->len + hdr_size(vc->td, hdr);
}
...
static int verify_io_u_xxhash(struct verify_header *hdr, struct vcont *vc)
{
	...
	struct vhdr_xxhash *vh = static_cast<struct vhdr_xxhash *>(hdr_priv(hdr));
	uint32_t hash;
	void *state;
			    ...
			    unsigned int sha_size, const char *name)
{
	...
	fio_sha3_update(sha3_ctx, static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	struct vhdr_sha3_224 *vh = static_cast<struct vhdr_sha3_224 *>(hdr_priv(hdr));
...
{
	struct vhdr_sha3_256 *vh = static_cast<struct vhdr_sha3_256 *>(hdr_priv(hdr));
...
{
	struct vhdr_sha3_384 *vh = static_cast<struct vhdr_sha3_384 *>(hdr_priv(hdr));
...
{
	struct vhdr_sha3_512 *vh = static_cast<struct vhdr_sha3_512 *>(hdr_priv(hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_sha512 *vh = static_cast<struct vhdr_sha512 *>(hdr_priv(hdr));
	uint8_t sha512[128];
	...
	fio_sha512_update(&sha512_ctx, static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_sha256 *vh = static_cast<struct vhdr_sha256 *>(hdr_priv(hdr));
	uint8_t sha256[64];
	...
	fio_sha256_update(&sha256_ctx, static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_sha1 *vh = static_cast<struct vhdr_sha1 *>(hdr_priv(hdr));
	uint32_t sha1[5];
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_crc7 *vh = static_cast<struct vhdr_crc7 *>(hdr_priv(hdr));
	unsigned char c;

	dprint(FD_VERIFY, "crc7 verify io_u %p, len %u\n", vc->io_u, hdr->len);

	c = fio_crc7(static_cast<unsigned char const *>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_crc16 *vh = static_cast<struct vhdr_crc16 *>(hdr_priv(hdr));
	unsigned short c;
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_crc64 *vh = static_cast<struct vhdr_crc64 *>(hdr_priv(hdr));
	unsigned long long c;

	dprint(FD_VERIFY, "crc64 verify io_u %p, len %u\n", vc->io_u, hdr->len);

	c = fio_crc64(static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_crc32 *vh = static_cast<struct vhdr_crc32 *>(hdr_priv(hdr));
	uint32_t c;
...
static int verify_io_u_crc32c(struct verify_header *hdr, struct vcont *vc)
{
	...
	c = fio_crc32c(static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
...
{
	void *p = io_u_verify_off(hdr, vc);
	struct vhdr_md5 *vh = static_cast<struct vhdr_md5 *>(hdr_priv(hdr));
	...
	fio_md5_update(&md5_ctx, static_cast<uint8_t const*>(p), hdr->len - hdr_size(vc->td, hdr));
			 ...
			 unsigned int hdr_len)
{
	...
	crc = fio_crc32c(static_cast<uint8_t const*>(p), offsetof(struct verify_header, crc32));
	if (crc != hdr->crc32) {
...
err:
	...
	if (td->o.verify_dump)
		dump_buf(static_cast<char*>(p), hdr_len, io_u->verify_offset + hdr_num * hdr_len,
				"hdr_fail", io_u->file);
...
int verify_io_u(struct thread_data *td, struct io_u **io_u_ptr)
{
	...
	ret = 0;
	for (auto *p = static_cast<char*>(io_u->buf); p < static_cast<char*>(io_u->buf) + io_u->buflen;
	     p += hdr_inc, hdr_num++) {
		struct vcont vc = {
			.io_u		= io_u,
			.hdr_num	= hdr_num,
...
{
	fio_sha3_update(sha3_ctx, static_cast<uint8_t const*>(p), len);
	fio_sha3_final(sha3_ctx);
}
...
static void fill_sha512(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	fio_sha512_init(&sha512_ctx);
	fio_sha512_update(&sha512_ctx, static_cast<uint8_t const*>(p), len);
}
...
static void fill_sha256(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	fio_sha256_init(&sha256_ctx);
	fio_sha256_update(&sha256_ctx, static_cast<uint8_t const*>(p), len);
	fio_sha256_final(&sha256_ctx);
}
...
static void fill_crc7(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	vh->crc7 = fio_crc7(static_cast<unsigned char const*>(p), len);
}
...
static void fill_crc32c(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	vh->crc32 = fio_crc32c(static_cast<unsigned char const*>(p), len);
}
...
static void fill_crc64(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	vh->crc64 = fio_crc64(static_cast<unsigned char const*>(p), len);
}
...
static void fill_md5(struct verify_header *hdr, void *p, unsigned int len)
{
	...
	fio_md5_init(&md5_ctx);
	fio_md5_update(&md5_ctx, static_cast<unsigned char const*>(p), len);
	fio_md5_final(&md5_ctx);
}
		       ...
		       unsigned int header_len, uint64_t rand_seed)
{
	...
	hdr->numberio = io_u->numberio;
	hdr->crc32 = fio_crc32c(static_cast<unsigned char const*>(p), offsetof(struct verify_header, crc32));
}
...
int get_next_verify(struct thread_data *td, struct io_u *io_u)
{
	...
	if (!RB_EMPTY_ROOT(&td->io_hist_tree)) {
		 ...
		 */
		if (__atomic_load_n(&ipo->flags, __ATOMIC_ACQUIRE) & IP_F_IN_FLIGHT)
			goto nothing;
...
int verify_async_init(struct thread_data *td)
{
	...
	td->verify_threads = static_cast<decltype(td->verify_threads)>(malloc(sizeof(pthread_t) * td->o.verify_async));
...
{
	struct io_u *io = static_cast<struct io_u *>(priv);
...
struct all_io_list *get_all_io_list(int save_mask, size_t *sz)
{
	...
	static_assert(sizeof(struct all_io_list) == 8, "all_io_list");
	...
	*sz += depth * sizeof(struct file_comp);
	rep = static_cast<decltype(rep)>(calloc(1, *sz));

	rep->threads = cpu_to_le64((uint64_t) nr);

	next = &rep->state[0];
	for_each_td(td) {
		struct thread_io_list *s = static_cast<struct thread_io_list *>(next);
				   ...
				   const char *prefix)
{
	...
	crc = fio_crc32c(reinterpret_cast<unsigned char const*>(s), thread_io_list_sz(s));
...
{
	struct thread_io_list *s = static_cast<struct thread_io_list *>(p);
	int i;
	...
	td->vstate = static_cast<decltype(td->vstate)>(p);
}
...
int verify_state_hdr(struct verify_state_hdr *hdr, struct thread_io_list *s)
{
	...
	crc = fio_crc32c(reinterpret_cast<unsigned char const *>(s), hdr->size);
	if (crc != hdr->crc)
		return 1;
