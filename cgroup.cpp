...
static struct cgroup_mnt *find_cgroup_mnt(struct thread_data *td)
{
	...
	if (mnt) {
		cgroup_mnt = static_cast<struct cgroup_mnt*>(smalloc(sizeof(*cgroup_mnt)));
		if (cgroup_mnt) {
			...
			struct flist_head *clist)
{
	...
	cm = static_cast<struct cgroup_member*>(smalloc(sizeof(*cm)));
	if (!cm) {
err:
...
static void fio_init cgroup_init()
{
...
static void fio_exit cgroup_exit()
{
	fio_sem_remove(lock);
}
