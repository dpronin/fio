...
static char *mkpath(const char *path, const char *name)
{
	...
	n = static_cast<char*>(xmalloc(len1 + len2 + 6));
...
libmtd_t libmtd_open()
{
	struct libmtd *lib;

	lib = static_cast<struct libmtd *>(xzalloc(sizeof(*lib)));
...
static int legacy_auto_oob_layout(const struct mtd_dev_info *mtd, int fd,
	...
	tmp_buf = static_cast<uint8_t*>(malloc(ooblen));
	...
	if (old_oobinfo.useecc == MTD_NANDECC_AUTOPLACE) {
		...
		for (i = 0; old_oobinfo.oobfree[i][1]; i++) {
			...
			memcpy(static_cast<char*>(oob) + start, tmp_buf + tags_pos, len);
			tags_pos += len;
		}
	} else {
		...
		memcpy(static_cast<char*>(oob) + start, tmp_buf + start, len);
	}
	free(tmp_buf);
	      ...
	      unsigned int cmd)
{
	...
	oob.ptr = static_cast<unsigned char*>(data);
		  ...
		  const char *img_name)
{
	...
	buf = static_cast<char*>(xmalloc(mtd->eb_size));
