...
static int proc_parse_start(struct proc_parse_info *pi)
{
	...
	pi->buf = static_cast<char*>(xmalloc(PROC_MTD_MAX_LEN));
...
static int proc_parse_next(struct proc_parse_info *pi)
{
	...
	p = static_cast<char*>(memchr(pi->next, '\"', pi->data_size - pos));
	if (!p)
	...
	p1 = static_cast<char*>(memchr(p, '\"', pi->data_size - pos));
	if (!p1)
