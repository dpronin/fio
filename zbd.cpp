...
static bool zbd_using_direct_io()
{
	struct fio_file *f;
	int j;
...
static int init_zone_info(struct thread_data *td, struct fio_file *f)
{
	...
	zbd_info = static_cast<decltype(zbd_info)>(scalloc(1, sizeof(*zbd_info) +
...
static int parse_zone_info(struct thread_data *td, struct fio_file *f)
{
	...
	zones = static_cast<decltype(zones)>(calloc(ZBD_REPORT_MAX_ZONES, sizeof(struct zbd_zone)));
	if (!zones)
		goto out;
