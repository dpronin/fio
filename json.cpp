...
struct json_object *json_create_object()
{
	return static_cast<json_object *>(calloc(1, sizeof(struct json_object)));
}

struct json_array *json_create_array()
{
	return static_cast<json_array *>(calloc(1, sizeof(struct json_array)));
}

static struct json_pair *json_create_pair(const char *name, struct json_value *value)
{
	struct json_pair *pair = static_cast<struct json_pair *>(malloc(sizeof(struct json_pair)));
	if (pair) {
...
{
	struct json_value *value = static_cast<struct json_value *>(malloc(sizeof(struct json_value)));
...
static char *strdup_escape(const char *str)
{
	...
	p = ret = static_cast<char*>(malloc(strlen(str) + escapes + 1));
	while (*str) {
		if (*str == '\\' || *str == '\"')
			*p++ = '\\';
		*p++ = *str++;
...
{
	struct json_value **values = static_cast<struct json_value **>(realloc(array->values,
...
{
	struct json_pair **pairs = static_cast<struct json_pair **>(realloc(obj->pairs,
