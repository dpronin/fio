			   ...
			   struct workqueue_work *work)
{
	...
	struct thread_data *td = static_cast<struct thread_data *>(sw->priv);
	int ret, error;
...
static int io_workqueue_alloc_fn(struct submit_worker *sw)
{
	...
	td = static_cast<struct thread_data *>(calloc(1, sizeof(*td)));
	sw->priv = td;
	return 0;
}
...
{
	struct thread_data *src = static_cast<struct thread_data *>(sw->priv);
