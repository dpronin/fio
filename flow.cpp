...
int flow_threshold_exceeded(struct thread_data *td)
{
	...
	flow_counter_ratio = (double)td->flow_counter /
		__atomic_load_n(&flow->flow_counter, __ATOMIC_RELAXED);
	flow_weight_ratio = (double)td->o.flow /
		__atomic_load_n(&flow->total_weight, __ATOMIC_RELAXED);
	...
	__atomic_fetch_add(&flow->flow_counter, 1, __ATOMIC_RELAXED);
	++td->flow_counter;
...
static struct fio_flow *flow_get(unsigned int id)
{
	...
	if (!flow) {
		flow = static_cast<decltype(flow)>(smalloc(sizeof(*flow)));
		if (!flow) {
				        ...
				        unsigned int weight)
{
	...
	__atomic_fetch_sub(&flow->flow_counter, flow_counter, __ATOMIC_RELAXED);
	__atomic_fetch_sub(&flow->total_weight, weight, __ATOMIC_RELAXED);
...
void flow_init_job(struct thread_data *td)
{
	if (td->o.flow) {
		...
		td->flow_counter = 0;
		__atomic_fetch_add(&td->flow->total_weight, td->o.flow, __ATOMIC_RELAXED);
	}
}
...
void flow_init()
{
	flow_list = static_cast<decltype(flow_list)>(smalloc(sizeof(*flow_list)));
	if (!flow_list) {
