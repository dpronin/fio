...
static bool pre_read_file(struct thread_data *td, struct fio_file *f)
{
	...
	b = static_cast<char*>(calloc(1, bs));
	if (!b) {
		td_verror(td, errno, "malloc");
		ret = false;
		goto error;
...
static unsigned long long get_fs_free_counts(struct thread_data *td)
{
	...
	for_each_file(td, f, i) {
		...
		fm = static_cast<decltype(fm)>(calloc(1, sizeof(*fm)));
...
static void set_already_allocated(const char *fname)
{
	...
	fn = static_cast<struct file_name *>(malloc(sizeof(struct file_name)));
...
static struct fio_file *alloc_new_file(struct thread_data *td)
{
	...
	if (td_ioengine_flagged(td, FIO_NOFILEHASH))
		f = static_cast<struct fio_file *>(calloc(1, sizeof(*f)));
	else
		f = static_cast<struct fio_file *>(scalloc(1, sizeof(*f)));
	if (!f) {
		assert(0);
		return NULL;
	}
...
int add_file(struct thread_data *td, const char *fname, int numjob, int inc)
{
	...
	if (td->files_size <= td->files_index) {
		...
		td->files = static_cast<decltype(td->files)>(realloc(td->files, new_size * sizeof(f)));
		if (td->files == NULL) {
			log_err("fio: realloc OOM\n");
			assert(0);
		}
		if (td->o.file_lock_mode != FILE_LOCK_NONE) {
			td->file_locks = static_cast<decltype(td->file_locks)>(realloc(td->file_locks, new_size));
			if (!td->file_locks) {
				log_err("fio: realloc OOM\n");
				assert(0);
			}
...
void dup_files(struct thread_data *td, struct thread_data *org)
{
	...
	td->files = static_cast<decltype(td->files)>(calloc(org->files_index, sizeof(f)));

	if (td->o.file_lock_mode != FILE_LOCK_NONE)
		td->file_locks = static_cast<decltype(td->file_locks)>(malloc(org->files_index));
