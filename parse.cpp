...
{
	const struct value_pair *vp1 = static_cast<const struct value_pair *>(p1);
	const struct value_pair *vp2 = static_cast<const struct value_pair *>(p2);
...
static void show_option_help(const struct fio_option *o, int is_err)
{
	const char *typehelp[] = {
		...
		"string (opt=bla)",
		"one to three ranges (opt=1k-4k[,4k-8k[,1k-8k]])",
...
extern "C" int evaluate_arithmetic_expression(const char *buffer, long long *ival,
...
{
	char delimiter[] = {',', ':'};
	for (unsigned int i = 0; i < FIO_ARRAY_SIZE(delimiter); i++) {
		if (auto const *postfix = strchr(str, delimiter[i]))
...
#define val_store(ptr, val, off, _or, data, o)		\
	do {						\
		ptr = static_cast<decltype(ptr)>(td_var((data), (o), (off)));	\
		if ((_or))				\
			*ptr |= (val);			\
		else					\
			*ptr = (val);			\
...
{
	static_assert(FIO_ARRAY_SIZE(opt_type_names) - 1 == FIO_OPT_UNSUPPORTED,
...
static int __handle_option(const struct fio_option *o, char *ptr,
			   void *data, int first, int more, int curr)
{
	...
	case FIO_OPT_STR_MULTI: {
		fio_opt_str_fn *fn = reinterpret_cast<fio_opt_str_fn*>(o->cb);
	...
	{
		fio_opt_str_val_fn *fn = reinterpret_cast<fio_opt_str_val_fn*>(o->cb);
		char tmp[128], *p;
...
store_option_value:
	...
	case FIO_OPT_FLOAT_LIST: {
		...
		if (first) {
			...
			*/
			if (o->off2) {
				ul2 = 0;
				ilp = static_cast<decltype(ilp)>(td_var(data, o, o->off2));
				*ilp = ul2;
			}

			flp = static_cast<decltype(flp)>(td_var(data, o, o->off1));
	...
	case FIO_OPT_STR_STORE: {
		auto *fn = reinterpret_cast<fio_opt_str_fn*>(o->cb);

		if (!strlen(ptr))
			return 1;

		if (o->off1) {
			cp = static_cast<decltype(cp)>(td_var(data, o, o->off1));
			if (*cp)
				free(*cp);
			*cp = strdup(ptr);
		...
		if (fn)
			...
			for (i = 0; i < PARSE_MAX_VP; i++) {
				...
				if (!strncmp(vp->ival, ptr, str_match_len(vp, ptr))) {
					...
					ret = 0;
					if (vp->cb)
						fn = reinterpret_cast<fio_opt_str_fn*>(vp->cb);
					rest = strstr(*cp ?: ptr, ":");
					if (rest) {
						if (*cp)
							*rest = '\0';
	...
	case FIO_OPT_STR_SET: {
		auto *fn = reinterpret_cast<fio_opt_int_fn*>(o->cb);
...
int string_distance(const char *s1, const char *s2)
{
	...
	p = static_cast<unsigned int*>(malloc(sizeof(unsigned int) * (s2_len + 1)));
	q = static_cast<unsigned int*>(malloc(sizeof(unsigned int) * (s2_len + 1)));
...
void options_mem_dupe(const struct fio_option *options, void *data)
{
	...
	for (o = &options[0]; o->name; o++) {
		...
		ptr = static_cast<char**>(td_var(data, o, o->off1));
		if (*ptr)
			*ptr = strdup(*ptr);
	}
}
