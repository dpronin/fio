...
static void __td_zone_gen_index(struct thread_data *td, enum fio_ddir ddir)
{
	...
	td->zone_state_index[ddir] = static_cast<struct zone_split_index*>(malloc(sizeof(struct zone_split_index) * 100));
...
void td_zone_gen_index(struct thread_data *td)
{
	...
	td->zone_state_index = static_cast<struct zone_split_index **>(malloc(DDIR_RWDIR_CNT *
					sizeof(struct zone_split_index *)));

	for (i = 0; i < DDIR_RWDIR_CNT; i++)
		__td_zone_gen_index(td, static_cast<enum fio_ddir>(i));
}
