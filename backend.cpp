...
static bool check_min_rate(struct thread_data *td, struct timespec *now)
{
	...
	for_each_rw_ddir(ddir) {
		if (td->bytes_done[ddir])
			ret |= __check_min_rate(td, now, static_cast<enum fio_ddir>(ddir));
	}
	...
	case FIO_Q_COMPLETED:
		if (io_u->error) {
			*ret = -io_u->error;
			...
			io_u->xfer_buf = reinterpret_cast<char*>(io_u->xfer_buf) + bytes;
			io_u->offset += bytes;
					...
					unsigned int depth)
{
	...
	for_each_file(td, f, i) {
		f->last_write_comp = static_cast<decltype(f->last_write_comp)>(scalloc(depth, sizeof(uint64_t)));
		if (!f->last_write_comp)
			goto cleanup;
	}
...
static int init_io_u(struct thread_data *td)
{
	...
	for (i = 0; i < max_units; i++) {
		...
		io_u = static_cast<decltype(io_u)>(ptr);
...
{
	struct fork_data *fd = static_cast<struct fork_data *>(data);
...
static bool __check_trigger_file()
{
	struct stat sb;
...
reap:
	...
	while (todo) {
		...
		for_each_td(td) {
			...
			fd = static_cast<decltype(fd)>(calloc(1, sizeof(*fd)));
			fd->td = td;
			fd->sk_out = sk_out;
...
int fio_backend(struct sk_out *sk_out)
{
	...
	cgroup_list = static_cast<decltype(cgroup_list)>(smalloc(sizeof(*cgroup_list)));
	if (cgroup_list)
