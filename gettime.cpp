...
static void fio_exit fio_dump_gtod()
{
...
static void fio_init gtod_init()
{
	int i;
...
static unsigned long get_cycles_per_msec()
{
	struct timespec s, e;
	uint64_t c_s, c_e;
	uint64_t elapsed;
...
static int calibrate_cpu_clock()
{
	double delta, mean, S;
	...
	maxc = samples = avg = 0;
	for (i = 0; i < NR_TIME_ITERS; i++) {
		double __this__ = cycles[i];

		minc = min(cycles[i], minc);
		maxc = max(cycles[i], maxc);

		if ((fmax(__this__, mean) - fmin(__this__, mean)) > S)
			continue;
		samples++;
		avg += __this__;
	}
...
static inline uint32_t atomic32_compare_and_swap(uint32_t *ptr, uint32_t old,
						 uint32_t __new__)
{
	return __sync_val_compare_and_swap(ptr, old, __new__);
}

static void *clock_thread_fn(void *data)
{
	struct clock_thread *t = static_cast<struct clock_thread *>(data);
...
{
	const struct clock_entry *c1 = static_cast<const struct clock_entry*>(p1);
	const struct clock_entry *c2 = static_cast<const struct clock_entry*>(p2);
...
int fio_monotonic_clocktest(int debug)
{
	...
	struct clock_entry *prev, *__this__;
	uint32_t seq = 0;
	unsigned int i;
	os_cpu_mask_t mask;
	...
	cthreads = static_cast<decltype(cthreads)>(malloc(nr_cpus * sizeof(struct clock_thread)));
	tentries = nr_entries * nr_cpus;
	entries = static_cast<decltype(entries)>(malloc(tentries * sizeof(struct clock_entry)));
	...
	for (failed = i = 0; i < tentries; i++) {
		...
		if (prev->tsc > __this__->tsc) {
			uint64_t diff = prev->tsc - __this__->tsc;
			...
			log_info("\t CPU%3u: TSC=%llu, SEQ=%u\n", __this__->cpu, (unsigned long long) __this__->tsc, __this__->seq);
			failed++;
		}
