...
static int alloc_mem_shm(struct thread_data *td, unsigned int total_mem)
{
	...
	td->orig_buffer = static_cast<decltype(td->orig_buffer)>(shmat(td->shm_id, NULL, 0));
...
static int alloc_mem_mmap(struct thread_data *td, size_t total_mem)
{
	...
	td->orig_buffer = static_cast<decltype(td->orig_buffer)>(mmap(NULL, total_mem, PROT_READ | PROT_WRITE, flags,
				td->mmapfd, 0));
...
{
	td->orig_buffer = static_cast<decltype(td->orig_buffer)>(malloc(total_mem));
