...
{
	if (shared)
		q->io_us = static_cast<decltype(q->io_us)>(smalloc(nr * sizeof(struct io_u *)));
	else
		q->io_us = static_cast<decltype(q->io_us)>(calloc(nr, sizeof(struct io_u *)));
...
bool io_u_rinit(struct io_u_ring *ring, unsigned int nr)
{
	...
	ring->ring = static_cast<decltype(ring->ring)>(calloc(ring->max, sizeof(struct io_u *)));
	if (!ring->ring)
		return false;
