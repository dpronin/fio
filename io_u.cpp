...
{
	auto const odir = static_cast<enum fio_ddir>(static_cast<int>(ddir) ^ 1);
	uint64_t usec;
	uint64_t now;
	...
	do {
		...
		if (!fio_file_open(f)) {
			...
			if (td->nr_open_files >= td->o.open_files)
				return static_cast<struct fio_file *>(ERR_PTR(-EBUSY));
...
again:
	...
	if (io_u) {
		...
		io_u->error = 0;
		io_u->acct_ddir = static_cast<decltype(io_u->acct_ddir)>(-1);
		td->cur_depth++;
		assert(!(td->flags & TD_F_CHILD));
...
static void small_content_scramble(struct io_u *io_u)
{
	...
	p = static_cast<char*>(io_u->xfer_buf);
	boffset = io_u->offset;
	...
	for (i = 0; i < nr_blocks; i++) {
		 ...
		 * of cacheline
		 */
		iptr = reinterpret_cast<uint64_t*>(p + (offset << 6));
		*iptr = boffset;

		iptr = reinterpret_cast<uint64_t*>(p + 64 - 2 * sizeof(uint64_t));
...
err_put:
	...
	put_io_u(td, io_u);
	return static_cast<struct io_u *>(ERR_PTR(ret));
}
			 ...
			 struct io_completion_data *icd)
{
	...
	if (io_u->ipo) {
		...
		if (io_u->error)
			...
			__atomic_store_n(&io_u->ipo->flags, io_u->ipo->flags & ~IP_F_IN_FLIGHT, __ATOMIC_RELEASE);
		}
	}
	...
	if (!io_u->error && ddir_rw(ddir)) {
		...
		if (bytes && io_u->resid) {
			...
			io_u->xfer_buf = static_cast<char*>(io_u->xfer_buf) + bytes;
			io_u->offset += bytes;
		    ...
		    unsigned long long max_bs)
{
	...
	if (o->compress_percentage || o->dedupe_percentage) {
		...
		do {
			...
			buf = static_cast<char*>(buf) + this_write;
			left -= this_write;
