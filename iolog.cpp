...
void log_io_piece(struct thread_data *td, struct io_u *io_u)
{
	...
	ipo = static_cast<decltype(ipo)>(calloc(1, sizeof(struct io_piece)));
	init_ipo(ipo);
	ipo->file = io_u->file;
...
static bool read_iolog(struct thread_data *td)
{
	 ...
	 */
	str = static_cast<char*>(malloc(4096));
	rfname = fname = static_cast<char*>(malloc(256 + 16));
	act = static_cast<char*>(malloc(256 + 16));
	...
	while ((p = fgets(str, 4096, td->io_log_rfile)) != NULL) {
		 ...
		 * Make note of file
		 */
		ipo = static_cast<decltype(ipo)>(calloc(1, sizeof(*ipo)));
		init_ipo(ipo);
		ipo->ddir = rw;
		if (td->io_log_version == 3)
			ipo->delay = delay;
	...
	td->o.td_ddir = static_cast<enum td_ddir>(0);
	if (reads)
		td->o.td_ddir = static_cast<enum td_ddir>(static_cast<int>(td->o.td_ddir) | TD_DDIR_READ);
	if (writes)
		td->o.td_ddir = static_cast<enum td_ddir>(static_cast<int>(td->o.td_ddir) | TD_DDIR_WRITE);
	if (trims)
		td->o.td_ddir = static_cast<enum td_ddir>(static_cast<int>(td->o.td_ddir) | TD_DDIR_TRIM);
...
static bool init_iolog_write(struct thread_data *td)
{
	...
	setvbuf(f, static_cast<char*>(td->iolog_buf), _IOFBF, 8192);
	       ...
	       const char *filename)
{
	...
	l = static_cast<decltype(l)>(scalloc(1, sizeof(*l)));
	...
	for (i = 0; i < DDIR_RWDIR_CNT; i++) {
		...
		entry = static_cast<decltype(entry)>(calloc(1, sizeof(struct io_u_plat_entry)));
	...
	if (l->td && l->td->o.io_submit_mode != IO_MODE_OFFLOAD) {
		...
		__p = static_cast<decltype(__p)>(calloc(1, sizeof(*l->pending)));
...
static void *set_file_buffer(FILE *f)
{
	...
	buf = malloc(size);
	setvbuf(f, static_cast<char*>(buf), _IOFBF, size);
	return buf;
}
...
void flush_samples(FILE *f, void *samples, uint64_t sample_size)
{
	...
	for (i = 0; i < nr_samples; i++) {
		...
		if (!log_offset) {
			...
			struct io_sample_offset *so = (struct io_sample_offset *) s;
...
static struct iolog_compress *get_new_chunk(unsigned int seq)
{
	...
	c = static_cast<struct iolog_compress*>(malloc(sizeof(*c)));
			    ...
			    z_stream *stream, struct inflate_chunk_iter *iter)
{
	...
	stream->next_in = static_cast<decltype(stream->next_in)>(ic->buf);
	...
	while (stream->avail_in) {
		...
		stream->next_out = static_cast<Bytef*>(iter->buf) + iter->buf_used;
	...
	ret = stream->next_in - static_cast<decltype(stream->next_in)>(ic->buf);
...
int iolog_file_inflate(const char *file)
{
	...
	do {
		...
		ic.seq++;
		ic.len -= iret;
		ic.buf  = static_cast<char*>(ic.buf) + iret;
	} while (1);
...
static int gz_work(struct iolog_flush_data *data)
{
	...
	stream.next_in = static_cast<decltype(stream.next_in)>(data->samples);
	...
	do {
		...
		stream.next_out = static_cast<decltype(stream.next_out)>(c->buf);
	...
	stream.next_out = static_cast<decltype(stream.next_out)>(c->buf) + c->len;
...
static int iolog_flush(struct io_log *log)
{
	...
	data = static_cast<struct iolog_flush_data *>(malloc(sizeof(*data)));
	if (!data)
		return 1;
...
int iolog_cur_flush(struct io_log *log, struct io_logs *cur_log)
{
	...
	data = static_cast<struct iolog_flush_data*>(smalloc(sizeof(*data)));
	if (!data)
		return 1;
...
static int __write_log(struct thread_data *td, struct io_log *log, int __try__)
{
	if (log)
		return finish_log(td, log, __try__);

	return 0;
}

static int write_iops_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->iops_log, __try__);
	if (!ret)
		td->iops_log = NULL;
...
static int write_slat_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->slat_log, __try__);
	if (!ret)
		td->slat_log = NULL;
...
static int write_clat_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->clat_log, __try__);
	if (!ret)
		td->clat_log = NULL;
...
static int write_clat_hist_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->clat_hist_log, __try__);
	if (!ret)
...
static int write_lat_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->lat_log, __try__);
	if (!ret)
		td->lat_log = NULL;
...
static int write_bandw_log(struct thread_data *td, int __try__, bool unit_log)
{
	int ret;
	...
	ret = __write_log(td, td->bw_log, __try__);
	if (!ret)
		td->bw_log = NULL;
