					    ...
					    const char *engine_lib)
{
	 ...
	 */
	ops = static_cast<decltype(ops)>(dlsym(dlhandle, engine_lib));
	if (!ops)
		ops = static_cast<decltype(ops)>(dlsym(dlhandle, "ioengine"));
	 ...
	 */
	if (!ops) {
		auto get_ioengine = reinterpret_cast<get_ioengine_t>(dlsym(dlhandle, "get_ioengine"));
