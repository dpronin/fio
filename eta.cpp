...
bool calc_thread_status(struct jobs_eta *je, int force)
{
	...
	eta_secs = static_cast<decltype(eta_secs)>(calloc(thread_number, sizeof(uint64_t)));
	...
	if (write_bw_log && rate_time > bw_avg_time && !any_td_in_ramp) {
		...
		for_each_rw_ddir(ddir) {
			add_agg_sample(sample_val(je->rate[ddir]), static_cast<enum fio_ddir>(ddir), 0);
		}
	}
	...
	if (je->eta_sec != INT_MAX && je->nr_running) {
		...
		for (ddir = 0; ddir < DDIR_RWDIR_CNT; ddir++) {
			rate_str[ddir] = num2str(je->rate[ddir], 4,
						1024, je->is_pow2, static_cast<enum n2s_unit>(je->unit_base));
...
void print_status_init(int thr_number)
{
	...
	static_assert(sizeof(struct jobs_eta) == sizeof(jep), "jobs_eta");
