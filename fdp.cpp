...
static int init_ruh_info(struct thread_data *td, struct fio_file *f)
{
	...
	ruhs = static_cast<struct fio_ruhs_info*>(scalloc(1, sizeof(*ruhs) + FDP_MAX_RUHS * sizeof(*ruhs->plis)));
	if (!ruhs)
		return -ENOMEM;
	...
	tmp = static_cast<struct fio_ruhs_info*>(scalloc(1, sizeof(*tmp) + ruhs->nr_ruhs * sizeof(*tmp->plis)));
	if (!tmp) {
		ret = -ENOMEM;
		goto out;
	}
