...
void sk_out_drop()
{
	...
	sk_out = static_cast<struct sk_out*>(pthread_getspecific(sk_out_key));
...
struct fio_net_cmd *fio_net_recv_cmd(int sk, bool wait)
{
	...
	do {
		...
		tmp = static_cast<struct fio_net_cmd*>(realloc(cmdret, cmd_size));
		if (!tmp) {
...
static uint64_t alloc_reply(uint64_t tag, uint16_t opcode)
{
	...
	reply = static_cast<struct fio_net_cmd_reply *>(calloc(1, sizeof(*reply)));
		     ...
		     uint64_t *tagptr, struct flist_head *list)
{
	...
	do {
		...
		if (!cmd || cur_len < sizeof(*cmd) + this_len) {
			...
			cmd = static_cast<struct fio_net_cmd*>(malloc(cur_len));
		}
		...
		size -= this_len;
		buf = static_cast<char const*>(buf) + this_len;
	} while (!ret && size);
					 ...
					 int flags)
{
	...
	entry = static_cast<struct sk_entry*>(smalloc(sizeof(*entry)));
	if (!entry)
		return NULL;
...
{
	struct sk_out *sk_out = static_cast<struct sk_out *>(pthread_getspecific(sk_out_key));
...
static void fio_server_add_fork_item(pid_t pid, struct flist_head *list)
{
	...
	ffi = static_cast<struct fio_fork_item*>(malloc(sizeof(*ffi)));
	ffi->exitval = 0;
	ffi->signal = 0;
	ffi->exited = 0;
	ffi->pid = pid;
...
static int handle_load_file_cmd(struct fio_net_cmd *cmd)
{
	...
	if (parse_jobs_ini(static_cast<char*>(file_name), 0, 0, pdu->client_type)) {
		fio_net_queue_quit();
		return -1;
	}
...
static int handle_job_cmd(struct fio_net_cmd *cmd)
{
	...
	if (parse_jobs_ini(static_cast<char*>(buf), 1, 0, pdu->client_type)) {
		fio_net_queue_quit();
		return -1;
	}
...
static int handle_jobline_cmd(struct fio_net_cmd *cmd)
{
	...
	clp = static_cast<struct cmd_line_pdu*>(pdu);
	clp->lines = le16_to_cpu(clp->lines);
	clp->client_type = le16_to_cpu(clp->client_type);
	argv = static_cast<char**>(malloc(clp->lines * sizeof(char *)));
	offset = sizeof(*clp);

	dprint(FD_NET, "server: %d command line args\n", clp->lines);

	for (i = 0; i < clp->lines; i++) {
		cslp = reinterpret_cast<struct cmd_single_line_pdu *>(static_cast<char*>(pdu) + offset);
...
static int handle_send_eta_cmd(struct fio_net_cmd *cmd)
{
	...
	if (!je) {
		size = sizeof(*je);
		je = static_cast<struct jobs_eta*>(calloc(1, size));
	} else {
...
#ifdef WIN32
static int handle_connection_process()
{
...
static int accept_loop(int listen_sk)
{
	...
	while (!exit_backend) {
		...
		sk_out = static_cast<struct sk_out*>(scalloc(1, sizeof(*sk_out)));
		if (!sk_out) {
			close(sk);
			return -1;
		}
...
int fio_server_text_output(int level, const char *buf, size_t len)
{
	...
	tlen = sizeof(*pdu) + len;
	pdu = static_cast<struct cmd_text_pdu*>(malloc(tlen));
...
void fio_server_send_ts(struct thread_stat *ts, struct group_run_stats *rs)
{
	...
	if (clat_prio_stats_extra_size) {
		for (i = 0; i < DDIR_RWDIR_CNT; i++) {
			...
			if (ts->nr_clat_prio[i]) {
				...
				struct cmd_ts_pdu *ptr = static_cast<struct cmd_ts_pdu *>(extended_buf);
	...
	if (ss_extra_size) {
		...
		/* ss bw */
		ss_bw = static_cast<uint64_t*>(extended_buf_wp);
		for (i = 0; i < ts->ss_dur; i++)
				...
				struct sk_entry *first)
{
	...
	stream->next_in = static_cast<Bytef*>(next_in);
	...
	do {
		if (!stream->avail_out) {
			...
			*out_pdu = malloc(FIO_SERVER_MAX_FRAGMENT_PDU);
			...
			stream->next_out = static_cast<Bytef*>(*out_pdu);
		}
				 ...
				 struct io_logs *cur_log, z_stream *stream)
{
	...
	stream->next_in = static_cast<Bytef*>(cur_log->log);
...
void fio_server_send_add_job(struct thread_data *td)
{
	...
	pdu = static_cast<struct cmd_add_job_pdu*>(malloc(cmd_sz));
				...
				void **datap)
{
	...
	rep = static_cast<struct cmd_reply *>(smalloc(sizeof(*rep)));
	if (!rep)
		return ENOMEM;
...
fail:
	 ...
	 */
	s = reinterpret_cast<struct thread_io_list*>((char*)rep->data + sizeof(struct verify_state_hdr));
	if (verify_state_hdr(static_cast<struct verify_state_hdr *>(rep->data), s)) {
		ret = EILSEQ;
		goto fail;
	}
...
static int fio_init_server_sock()
{
...
static int fio_init_server_connection()
{
	char bind_str[128];
	int sk;
			    ...
			    struct in6_addr *inp6, int *ipv6)
{
	...
	*ipv6 = 0;
	 ...
	 */
	if (!lport) {
		portp = strchr(const_cast<char*>(host), ',');
		if (portp) {
			*portp = '\0';
			portp++;
			lport = atoi(portp);
 ...
 *
 */
static int fio_handle_server_arg()
{
...
void fio_server_destroy_sk_key()
{
