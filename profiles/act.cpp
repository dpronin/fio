...
{
	struct act_prof_data *apd = static_cast<struct act_prof_data *>(td->prof_data);
...
static void act_show_all_stats()
{
...
static void put_act_ref(struct thread_data *td)
{
	...
	if (!act_run_data->slices) {
		act_run_data->slices = static_cast<decltype(act_run_data->slices)>(calloc(apd->nr_slices, sizeof(struct act_slice)));
...
static int act_td_init(struct thread_data *td)
{
	...
	apd = static_cast<decltype(apd)>(calloc(1, sizeof(*apd)));
	nr_slices = (act_options.test_duration + SAMPLE_SEC - 1) / SAMPLE_SEC;
	apd->slices = static_cast<decltype(apd->slices)>(calloc(nr_slices, sizeof(struct act_slice)));
...
static void fio_init act_register()
{
	act_run_data = static_cast<decltype(act_run_data)>(calloc(1, sizeof(*act_run_data)));
...
static void fio_exit act_unregister()
{
