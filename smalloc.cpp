...
static inline int ptr_valid(struct pool *pool, void *ptr)
{
	...
	return (ptr >= pool->map) && (ptr < static_cast<const char*>(pool->map) + pool_size);
}
...
{
	unsigned int *postred = static_cast<unsigned int *>(postred_ptr(hdr));
...
static void sfree_pool(struct pool *pool, void *ptr)
{
	...
	ptr = static_cast<char*>(ptr) - sizeof(*hdr);
	hdr = static_cast<struct block_hdr *>(ptr);

	assert(ptr_valid(pool, ptr));

	sfree_check_redzone(hdr);

	offset = static_cast<char*>(ptr) - static_cast<char*>(pool->map);
...
static void *__smalloc_pool(struct pool *pool, size_t size)
{
	...
	if (i < pool->nr_blocks) {
		...
		ret = static_cast<char*>(pool->map) + offset;
	}
fail:
	fio_sem_up(pool->lock);
	return ret;
...
static void *smalloc_pool(struct pool *pool, size_t size)
{
	...
	if (ptr) {
		struct block_hdr *hdr = static_cast<struct block_hdr *>(ptr);
...
static void smalloc_print_bitmap(struct pool *pool)
{
	...
	buffer = static_cast<char*>(malloc(SMALLOC_BPI + 1));
	if (!buffer)
		return;
...
char *smalloc_strdup(const char *str)
{
	...
	ptr = static_cast<char *>(smalloc(strlen(str) + 1));
	if (ptr)
		strcpy(ptr, str);
	return ptr;
}
