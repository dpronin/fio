...
{
	struct submit_worker *sw = static_cast<struct submit_worker *>(data);
	struct workqueue *wq = sw->wq;
	unsigned int ret = 0;
		   ...
		   struct sk_out *sk_out)
{
	...
	wq->workers = static_cast<decltype(wq->workers)>(smalloc(wq->max_workers * sizeof(struct submit_worker)));
	if (!wq->workers)
		goto err;
