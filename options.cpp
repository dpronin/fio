 ...
 */
static char *get_opt_postfix(char *str)
{
	char *p = strstr(str, ":");
...
{
	auto const *bsp1 = static_cast<const struct bssplit*>(p1);
	auto const *bsp2 = static_cast<const struct bssplit*>(p2);
			...
			enum fio_ddir ddir, char *str, bool data)
{
	...
	max_bs = 0;
	min_bs = -1;
	o->bssplit[ddir] = static_cast<struct bssplit*>(malloc(split.nr * sizeof(struct bssplit)));
...
{
	const uint16_t *t1 = static_cast<const uint16_t*>(p1);
	const uint16_t *t2 = static_cast<const uint16_t*>(p2);
...
{
	auto *td = static_cast<struct thread_data*>(cb_data_to_td(data));
	char *str, *p;
	int ret = 0;
				       ...
				       struct split_prio *entry, char *str)
{
	...
	char *bs_str = NULL;
	long long bs_val;
	unsigned int perc = 0, class__, level, hint;
	...
	matches = sscanf(str, "%m[^/]/%u/%u/%u/%u",
			 &bs_str, &perc, &class__, &level, &hint);
	if (matches < 1) {
	...
	case 5: /* bs/perc/class/level/hint case */
		class__ = min(class__, (unsigned int) IOPRIO_MAX_PRIO_CLASS);
		level = min(level, (unsigned int) IOPRIO_MAX_PRIO);
		if (matches == 5)
			hint = min(hint, (unsigned int) IOPRIO_MAX_PRIO_HINT);
		else
			hint = 0;
		entry->prio = ioprio_value(class__, level, hint);
		break;
	default:
		log_err("fio: invalid cmdprio_bssplit format\n");
		return 1;
...
{
	auto const *tmp1 = static_cast<const struct split_prio*>(p1);
	auto const *tmp2 = static_cast<const struct split_prio*>(p2);
			  ...
			  int *nr_entries, char *str)
{
	...
	tmp_entries = static_cast<decltype(tmp_entries)>(calloc(nr_bssplits, sizeof(*tmp_entries)));
	if (!tmp_entries)
		return 1;
				...
				char *str)
{
	...
	error = static_cast<decltype(error)>(calloc(4, sizeof(int)));
	...
	while ((fname = strsep(&str, ":")) != NULL) {
		...
		if (i == td->o.ignore_error_nr[etype]) {
			...
			error = static_cast<decltype(error)>(realloc(error, td->o.ignore_error_nr[etype]
						  * sizeof(int)));
		}
		if (fname[0] == 'E') {
	...
	if (i) {
		td->o.continue_on_error = static_cast<decltype(td->o.continue_on_error)>(td->o.continue_on_error | 1 << etype);
...
static int str_ignore_error_cb(void *data, const char *input)
{
	...
	while (p) {
		...
		if (n)
			*n++ = '\0';
		ret = ignore_error_type(td, static_cast<enum error_type_bit>(type), p);
		if (ret)
			break;
		p = n;
		type++;
...
static int str_rw_cb(void *data, char *str)
{
	struct thread_data *td = cb_data_to_td(data);
...
static int str_fst_cb(void *data, char *str)
{
	struct thread_data *td = cb_data_to_td(data);
	double val;
	double center = -1;
...
static int str_sfr_cb(void *data, char *str)
{
	struct thread_data *td = cb_data_to_td(data);
			   ...
			   enum fio_ddir ddir, char *str, bool absolute)
{
	...
	o->zone_split[ddir] = static_cast<struct zone_split*>(malloc(split.nr * sizeof(struct zone_split)));
...
static int str_random_distribution_cb(void *data, char *str)
{
	struct thread_data *td = cb_data_to_td(data);
	double val;
	double center = -1;
...
static int str_steadystate_cb(void *data, char *str)
{
	struct thread_data *td = cb_data_to_td(data);
	double val;
	char *nr;
...
struct fio_option fio_options[FIO_MAX_OPTS] = {
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_filename_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_directory_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_opendir_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_rw_cb),
		.help	= "IO direction",
		.def	= "read",
		.posval = {
			  { .ival = "read",
...

	{
		...
		.posval	= {
			  ...
			  { .ival = "external",
			    ...
			    .cb = reinterpret_cast<void*>(str_ioengine_external_cb),
			  },
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_size_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_io_size_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_offset_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_offset_increment_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_bssplit_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_random_distribution_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_fst_cb),
...
#ifdef CONFIG_SYNC_FILE_RANGE
	{
		...
		.cb	= reinterpret_cast<void*>(str_sfr_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(fio_clock_source_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_mem_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_verify_pattern_cb),
...
#ifdef FIO_HAVE_CPU_AFFINITY
	{
		...
		.cb	= reinterpret_cast<void*>(str_verify_cpus_allowed_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_replay_skip_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_zoneskip_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_fdp_pli_cb),
	...
	},
	{
		...
		.maxval	= 100,
		.interval = 5,
		.cb	= reinterpret_cast<void*>(str_rwmix_read_cb),
	...
	},
	{
		...
		.maxval	= 100,
		.interval = 5,
		.cb	= reinterpret_cast<void*>(str_rwmix_write_cb),
...
#error "FIO_HAVE_IOPRIO_CLASS requires FIO_HAVE_IOPRIO"
...
#ifdef FIO_HAVE_CPU_AFFINITY
	{
		...
		.cb	= reinterpret_cast<void*>(str_cpumask_cb),
	...
	},
	{
		...
		.type	= FIO_OPT_STR,
		.off1	= offsetof(struct thread_options, cpumask),
		.cb	= reinterpret_cast<void*>(str_cpus_allowed_cb),
...
#ifdef CONFIG_LIBNUMA
	{
		...
		.cb	= reinterpret_cast<void*>(str_numa_cpunodes_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_numa_mpol_cb),
	...
	},
	{
		...
		.type	= FIO_OPT_STR_SET,
		.cb	= reinterpret_cast<void*>(str_exitall_cb),
	...
	},
	{
		...
		.posval	= {
			  ...
			  { .ival = "all",
			    .oval = static_cast<unsigned long long>(TERMINATE_ALL),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_write_bw_log_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_write_lat_log_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_write_iops_log_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_write_hist_log_cb),
...
#ifdef FIO_HAVE_CPU_AFFINITY
	{
		...
		.cb	= reinterpret_cast<void*>(str_log_cpus_allowed_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_buffer_pattern_cb),
		.help	= "Fill pattern for IO buffers",
		.category = FIO_OPT_C_IO,
		.group	= FIO_OPT_G_IO_BUF,
	},
	{
		...
		.minval	= 0,
		.interval = 5,
		.cb	= reinterpret_cast<void*>(str_buffer_compress_cb),
	...
	},
	{
		...
		.minval	= 0,
		.interval = 1,
		.cb	= reinterpret_cast<void*>(str_dedupe_cb),
...
#endif
	{
		...
		.type	= FIO_OPT_BOOL,
		.cb	= reinterpret_cast<void*>(str_gtod_reduce_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_ignore_error_cb),
	...
	},
	{
		...
		.cb	= reinterpret_cast<void*>(str_steadystate_cb),
...
static char *fio_keyword_replace(char *opt)
{
	...
	for (i = 0; fio_keywords[i].word != NULL; i++) {
		...
		while ((s = strstr(opt, kw->word)) != NULL) {
			char *__new__ = static_cast<char*>(calloc(strlen(opt) + 1, 1));
			char *o_org = opt;
			int olen = s - opt;
			int len;
			 ...
			 */
			memcpy(__new__, opt, olen);
			len = sprintf(__new__ + olen, "%s", kw->replace);
			...
			if (strlen(opt))
				memcpy(__new__ + olen + len, opt, strlen(opt));
...
{
	int i;
	char **opts_copy = static_cast<char**>(malloc(num_opts * sizeof(*opts)));
