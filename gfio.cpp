				   ...
				   gpointer data)
{
	...
	struct gfio_graphs *g = static_cast<struct gfio_graphs *>(data);
...
{
	struct gfio_graphs *g = static_cast<struct gfio_graphs *>(p);
	cairo_t *cr;
...
{
	struct gui *ui = static_cast<struct gui *>(arg);
...
static void start_job_clicked(__attribute__((unused)) GtkWidget *widget,
			      gpointer data)
{
	struct gui_entry *ge = static_cast<struct gui_entry *>(data);
	struct gfio_client *gc = ge->client;
...
{
	struct connection_widgets *cw = static_cast<struct connection_widgets *>(data);
...
static void gfio_client_added(struct gui_entry *ge, struct fio_client *client)
{
	...
	gc = static_cast<struct gfio_client *>(calloc(1, sizeof(*gc)));
	 ...
	 * to handle this
	 */
	gco = static_cast<struct gfio_client_options *>(calloc(1, sizeof(*gco)));
...
static struct gui_entry *alloc_new_gui_entry(struct gui *ui)
{
	...
	ge = static_cast<struct gui_entry *>(calloc(1, sizeof(*ge)));
					  ...
					  int *created)
{
	...
	return static_cast<struct gui_entry *>(g_hash_table_lookup(ui->ge_hash, &cur_page));
}
...
static void recent_open(GtkAction *action, gpointer data)
{
	...
	info = static_cast<GtkRecentInfo*>(g_object_get_data(G_OBJECT(action), "gtk-recent-info"));
...
static void file_open(GtkWidget *w, gpointer data)
{
	struct gui *ui = static_cast<struct gui *>(data);
	GtkWidget *dialog;
...
static void update_graph_limits()
{
...
static void preferences(GtkWidget *w, gpointer data)
{
	...
	buttons = static_cast<GtkWidget**>(malloc(sizeof(GtkWidget *) * FD_DEBUG_MAX));
...
static void add_recent_file_items(struct gui *ui)
{
	...
	for (item = items; item && item->data; item = g_list_next(item)) {
		...
		action = static_cast<GtkAction *>(g_object_new(GTK_TYPE_ACTION,
					"name", action_name,
					"label", label, NULL));
