				   ...
				   unsigned long long *maxv, unsigned long long *minv)
{
	...
	*maxv = 0;
	...
	ovals = static_cast<decltype(ovals)>(malloc(len * sizeof(*ovals)));
	if (!ovals)
		return 0;
...
void show_mixed_group_stats(struct group_run_stats *rs, struct buf_output *out) 
{
	...
	agg = num2str(agg_mix, rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
	aggalt = num2str(agg_mix, rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
	min = num2str(min_mix, rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
	minalt = num2str(min_mix, rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
	max = num2str(max_mix, rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
	maxalt = num2str(max_mix, rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
...
void show_group_stats(struct group_run_stats *rs, struct buf_output *out)
{
	...
	for (i = 0; i < DDIR_RWDIR_CNT; i++) {
		...
		agg = num2str(rs->agg[i], rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
		aggalt = num2str(rs->agg[i], rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
		min = num2str(rs->min_bw[i], rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
		minalt = num2str(rs->min_bw[i], rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
		max = num2str(rs->max_bw[i], rs->sig_figs, 1, i2p, static_cast<enum n2s_unit>(rs->unit_base));
		maxalt = num2str(rs->max_bw[i], rs->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(rs->unit_base));
...
static struct thread_stat *gen_mixed_ddir_stats_from_ts(struct thread_stat *ts)
{
	 ...
	 * Trims (ddir = 2)
	 */
	ts_lcl = static_cast<decltype(ts_lcl)>(malloc(sizeof(struct thread_stat)));
	if (!ts_lcl) {
			     ...
			     enum fio_ddir ddir, struct buf_output *out)
{
	...
	bw_p = num2str(bw, ts->sig_figs, 1, i2p, static_cast<enum n2s_unit>(ts->unit_base));
	bw_p_alt = num2str(bw, ts->sig_figs, 1, !i2p, static_cast<enum n2s_unit>(ts->unit_base));
				  ...
				  unsigned int *types)
{
	...
	*percentiles = static_cast<unsigned int*>(calloc(len, sizeof(**percentiles)));
...
static void show_ss_normal(struct thread_stat *ts, struct buf_output *out)
{
	...
	p1 = num2str(bw_mean / ts->kb_base, ts->sig_figs, ts->kb_base, i2p, static_cast<enum n2s_unit>(ts->unit_base));
	p1alt = num2str(bw_mean / ts->kb_base, ts->sig_figs, ts->kb_base, !i2p, static_cast<enum n2s_unit>(ts->unit_base));
				      ...
				      struct buf_output *out)
{
	...
	for_each_rw_ddir(ddir) {
		if (ts->io_bytes[ddir])
			show_ddir_status(rs, ts, static_cast<enum fio_ddir>(ddir), out);
	}
			      ...
			      int nr_prios)
{
	...
	clat_prio = static_cast<decltype(clat_prio)>(scalloc(nr_prios, sizeof(*ts->clat_prio[ddir])));
	if (!clat_prio) {
...
static int grow_clat_prio_stat(struct thread_stat *dst, enum fio_ddir ddir)
{
	...
	dst->clat_prio[ddir] = static_cast<struct clat_prio_stat *>(new_arr);
				...
				enum fio_ddir dst_ddir, enum fio_ddir src_ddir)
{
	...
	dst->clat_prio[dst_ddir] = static_cast<struct clat_prio_stat*>(smalloc(sz));
...
void sum_thread_stats(struct thread_stat *dst, struct thread_stat *src)
{
	...
	for (l = 0; l < DDIR_RWDIR_CNT; l++) {
		if (dst->unified_rw_rep != UNIFIED_MIXED) {
			...
			sum_clat_prio_stats(dst, src, static_cast<enum fio_ddir>(l), static_cast<enum fio_ddir>(l));
			...
			sum_clat_prio_stats(dst, src, static_cast<enum fio_ddir>(0), static_cast<enum fio_ddir>(l));
...
void __show_run_stats()
{
	...
	runstats = static_cast<decltype(runstats)>(malloc(sizeof(struct group_run_stats) * (groupid + 1)));
	...
	threadstats = static_cast<decltype(threadstats)>(malloc(nr_ts * sizeof(struct thread_stat)));
	opt_lists = static_cast<decltype(opt_lists)>(malloc(nr_ts * sizeof(struct flist_head *)));
...
int __show_running_run_stats()
{
	...
	rt = static_cast<unsigned long long *>(malloc(thread_number * sizeof(unsigned long long)));
	fio_gettime(&ts, NULL);
...
static struct io_logs *get_new_log(struct io_log *iolog)
{
	...
	cur_log = static_cast<decltype(cur_log)>(smalloc(sizeof(*cur_log)));
	if (cur_log) {
			     ...
			     unsigned int priority)
{
	...
	if (cur_log) {
		...
		if (iolog->log_offset) {
			struct io_sample_offset *so = reinterpret_cast<struct io_sample_offset *>(s);
...
{
	for (int ddir = 0; ddir < DDIR_RWDIR_CNT; ddir++)
		__add_stat_to_log(iolog, static_cast<enum fio_ddir>(ddir), elapsed, log_max);
}
		     ...
		     unsigned short clat_prio_index)
{
	...
	if (iolog && iolog->hist_msec) {
		...
		if (this_window >= iolog->hist_msec) {
			...
			dst = static_cast<decltype(dst)>(malloc(sizeof(struct io_u_plat_entry)));
			 ...
			 bool is_kb)
{
	...
	for (ddir = 0; ddir < DDIR_RWDIR_CNT; ddir++) {
		...
		if (log) {
			...
			next = add_log_sample(td, log, sample_val(rate), static_cast<enum fio_ddir>(ddir),
					      bs, 0, 0);
