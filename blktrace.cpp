...
static void trace_add_open_close_event(struct thread_data *td, int fileno, enum file_log_act action)
{
	...
	ipo = static_cast<struct io_piece *>(calloc(1, sizeof(*ipo)));
	init_ipo(ipo);
...
int merge_blktrace_iologs(struct thread_data *td)
{
	...
	struct blktrace_cursor *bcs = static_cast<struct blktrace_cursor *>(malloc(sizeof(struct blktrace_cursor) *
					     nr_logs));
	...
	merge_buf = static_cast<char*>(malloc(128 * 1024));
	if (!merge_buf)
		goto err_out_file;
