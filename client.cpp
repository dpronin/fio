...
static void fio_init fio_client_hash_init()
{
	int i;
...
static void fio_client_json_init()
{
	char time_buf[32];
	time_t time_p;
...
static void fio_client_json_fini()
{
					...
					const char *opt)
{
	...
	index = client->argc++;
	client->argv = static_cast<char**>(realloc(client->argv, sizeof(char *) * client->argc));
...
{
	struct fio_client *client = static_cast<struct fio_client *>(cookie);
	struct flist_head *entry;
...
static struct fio_client *get_new_client()
{
	struct fio_client *client;

	client = static_cast<struct fio_client *>(calloc(1, sizeof(*client)));
...
int fio_client_add_ini_file(void *cookie, const char *ini_file, bool remote)
{
	...
	client->files = static_cast<decltype(client->files)>(new_files);
...
{
	struct fio_client *existing = static_cast<struct fio_client *>(*cookie);
	struct fio_client *client;
...
static void fio_clients_terminate()
{
	struct flist_head *entry;
	struct fio_client *client;
...
static void client_signal_handler()
{
...
static int send_client_cmd_line(struct fio_client *client)
{
	...
	lens = static_cast<unsigned int*>(malloc(client->argc * sizeof(unsigned int)));
	...
	for (i = 0; i < client->argc; i++) {
		...
		cslp = reinterpret_cast<struct cmd_single_line_pdu *>((char*)pdu + offset);
					...
					const char *filename)
{
	...
	pdu = static_cast<struct cmd_load_file_pdu *>(calloc(1, p_size));
				       ...
				       const char *filename)
{
	...
	pdu = static_cast<struct cmd_job_pdu *>(malloc(p_size));
	buf = pdu->buf;
...
static void handle_job_opt(struct fio_client *client, struct fio_net_cmd *cmd)
{
	...
	if (pdu->global) {
		...
		p = static_cast<struct print_option*>(malloc(sizeof(*p)));
				   ...
				   struct fio_net_cmd *cmd)
{
	...
	log_pathname = static_cast<char*>(malloc(10 + strlen((char *)pdu->name) +
...
static void handle_start(struct fio_client *client, struct fio_net_cmd *cmd)
{
	...
	if (client->jobs) {
		...
		client->opt_lists = static_cast<struct flist_head*>(malloc(client->jobs * sizeof(struct flist_head)));
					      ...
					      struct cmd_iolog_pdu *pdu)
{
	...
	if (pdu->log_type == IO_LOG_TYPE_HIST)
		...
		total = nr_samples * __log_entry_sz(le32_to_cpu(pdu->log_offset));
	ret = static_cast<struct cmd_iolog_pdu *>(malloc(total + sizeof(*pdu)));
	...
	stream.next_in = reinterpret_cast<Bytef *>((char *) pdu + sizeof(*pdu));
	while (stream.avail_in) {
		...
		stream.next_out = reinterpret_cast<Bytef*>(p);
			 ...
			 uint64_t tag)
{
	...
	size = sizeof(*rep);
	rep = static_cast<struct cmd_sendfile_reply*>(malloc(size));
...
fail:
	...
	size += sb.st_size;
	rep = static_cast<struct cmd_sendfile_reply *>(realloc(rep, size));
...
int fio_clients_send_trigger(const char *cmd)
{
	...
	flist_for_each(entry, &client_list) {
		...
		pdu = static_cast<struct cmd_vtrigger_pdu *>(malloc(sizeof(*pdu) + slen));
...
static void request_client_etas(struct client_ops const *ops)
{
	...
	eta = static_cast<struct client_eta *>(calloc(1, sizeof(*eta) + __THREAD_RUNSTR_SZ(REAL_MAX_JOBS)));
...
static int fio_check_clients_timed_out()
{
	struct fio_client *client;
	struct flist_head *entry, *tmp;
	struct timespec ts;
...
int fio_handle_clients(struct client_ops const *ops)
{
	...
	pfds = static_cast<struct pollfd *>(malloc(nr_clients * sizeof(struct pollfd)));
