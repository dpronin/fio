...
#ifndef CONFIG_NO_SHM
	seg->threads = static_cast<decltype(seg->threads)>(shmat(seg->shm_id, NULL, 0));
...
static void copy_opt_list(struct thread_data *dst, struct thread_data *src)
{
	...
	flist_for_each(entry, &src->opt_list) {
		...
		dstp = static_cast<struct print_option *>(malloc(sizeof(*dstp)));
...
static int setup_rate(struct thread_data *td)
{
	...
	for_each_rw_ddir(ddir) {
		if (td->o.rate[ddir] || td->o.rate_iops[ddir]) {
			ret |= __setup_rate(td, static_cast<enum fio_ddir>(ddir));
		}
	}
	return ret;
}
...
static void init_flags(struct thread_data *td)
{
	...
	for (i = 0; i < DDIR_RWDIR_CNT; i++) {
		if (option_check_rate(td, static_cast<enum fio_ddir>(i))) {
		   ...
		   int recursed, int client_type)
{
	...
	if (o->write_lat_log) {
		struct log_params p = {
			...
			.hist_coarseness = static_cast<int>(o->log_hist_coarseness),
			.log_type = IO_LOG_TYPE_LAT,
			.log_offset = static_cast<int>(o->log_offset),
			.log_prio = static_cast<int>(o->log_prio),
			.log_gz = static_cast<int>(o->log_gz),
			.log_gz_store = static_cast<int>(o->log_gz_store),
		};
		...
		int nested, char *name, char ***popts, int *aopts, int *nopts)
{
	...
	string = static_cast<char*>(malloc(OPT_LEN_MAX));

	/*
	 * it's really 256 + small bit, 280 should suffice
	 */
	if (!nested) {
		name = static_cast<char*>(calloc(1, 280));
	}
	...
	if (!opts) {
		alloc_opts = 8;
		opts = static_cast<char**>(malloc(sizeof(char *) * alloc_opts));
		num_opts = 0;
	}
	...
	do {
		...
		while (1) {
			...
			if (num_opts == alloc_opts) {
				alloc_opts <<= 1;
				opts = static_cast<char**>(realloc(opts,
...
static void show_debug_categories()
{
#ifdef FIO_INC_DEBUG
...
static void fio_options_fill_optstring()
{
...
int parse_cmd_line(int argc, char *argv[], int client_type)
{
	...
	while ((c = getopt_long_only(argc, argv, ostr, l_opts, &lidx)) != -1) {
		...
		case 'x': {
			...
			job_sections = static_cast<char**>(realloc(job_sections, new_size));
		...
		case 'p':
			...
			break;
		case static_cast<decltype(c)>(FIO_GETOPT_JOB): {
		...
		case static_cast<decltype(c)>(FIO_GETOPT_IOENGINE): {
			const char *opt = l_opts[lidx].name;
			char *val = optarg;
	...
	while (!ret && optind < argc) {
		ini_idx++;
		ini_file = static_cast<char**>(realloc(ini_file, ini_idx * sizeof(char *)));
...
struct thread_data *get_global_options()
{
	return &def_thread;
}
