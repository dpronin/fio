...
{
	struct helper_data *hd = static_cast<struct helper_data *>(data);
	...
	struct interval_timer timer[] = {
		...
		},
		{
			...
			.interval_ms = static_cast<uint32_t>(status_interval),
			.func = __show_running_run_stats,
		},
		{
			.name = "steadystate",
			.interval_ms = static_cast<uint32_t>(steadystate_enabled ? ss_check_interval :
				0),
...
int helper_thread_create(struct fio_sem *startup_sem, struct sk_out *sk_out)
{
	...
	hd = static_cast<struct helper_data *>(scalloc(1, sizeof(*hd)));
