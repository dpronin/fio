				       ...
				       int mindev, char *path)
{
	...
	du = static_cast<struct disk_util*>(smalloc(sizeof(*du)));
	if (!du)
		return NULL;
...
void init_disk_util(struct thread_data *td)
{
	...
	if (!td->o.do_disk_util ||
	    td_ioengine_flagged(td, static_cast<enum fio_ioengine_flags>(FIO_DISKLESSIO | FIO_NODISKUTIL)))
		return;
