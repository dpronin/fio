...
struct graph *graph_new(unsigned int xdim, unsigned int ydim, const char *font)
{
	...
	g = static_cast<struct graph*>(calloc(1, sizeof(*g)));
...
{
	struct xyvalue *xy = static_cast<struct xyvalue *>(v->value);
	return xy->x;
}
...
graph_label_t graph_add_label(struct graph *bg, const char *label)
{
	...
	i = static_cast<struct graph_label*>(calloc(1, sizeof(*i)));
				  ...
				  const char *tooltip)
{
	...
	x = static_cast<struct graph_value *>(calloc(1, sizeof(*x)));
...
int graph_add_data(struct graph *bg, graph_label_t label, const double value)
{
	...
	d = static_cast<double*>(malloc(sizeof(*d)));
	*d = value;
		      ...
		      const double x, const double y, const char *tooltip)
{
	...
	xy = static_cast<struct xyvalue *>(malloc(sizeof(*xy)));
	xy->x = x;
	xy->y = y;
