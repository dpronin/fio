				  ...
				  size_t top_sz)
{
	...
	o->td_ddir = static_cast<enum td_ddir>(le32_to_cpu(top->td_ddir));
	...
	for (i = 0; i < DDIR_RWDIR_CNT; i++) {
		...
		if (o->bssplit_nr[i]) {
			o->bssplit[i] = static_cast<struct bssplit*>(malloc(o->bssplit_nr[i] * sizeof(struct bssplit)));
		...
		if (o->zone_split_nr[i]) {
			o->zone_split[i] = static_cast<struct zone_split*>(malloc(o->zone_split_nr[i] * sizeof(struct zone_split)));
	...
	o->file_lock_mode = static_cast<enum file_lock_mode>(le32_to_cpu(top->file_lock_mode));
	...
	o->verify_pattern = static_cast<char*>(realloc(o->verify_pattern,
				    o->verify_pattern_bytes));
	o->buffer_pattern = static_cast<char*>(realloc(o->buffer_pattern,
				    o->buffer_pattern_bytes));
	...
	o->zone_mode = static_cast<enum fio_zone_mode>(le32_to_cpu(top->zone_mode));
	...
	o->mem_type = static_cast<enum fio_memtype>(le32_to_cpu(top->mem_type));
	...
	o->fallocate_mode = static_cast<enum fio_fallocate_mode>(le32_to_cpu(top->fallocate_mode));
	...
	o->clocksource = static_cast<enum fio_cs>(le32_to_cpu(top->clocksource));
	...
	o->continue_on_error = static_cast<enum error_type>(le32_to_cpu(top->continue_on_error));
				   ...
				   struct thread_options *o)
{
	...
	top->td_ddir = cpu_to_le32(static_cast<uint32_t>(o->td_ddir));
	...
	top->file_lock_mode = cpu_to_le32(static_cast<uint32_t>(o->file_lock_mode));
	...
	top->mem_type = cpu_to_le32(static_cast<uint32_t>(o->mem_type));
	...
	top->fallocate_mode = cpu_to_le32(static_cast<uint32_t>(o->fallocate_mode));
	...
	top->clocksource = cpu_to_le32(static_cast<uint32_t>(o->clocksource));
	...
	top->continue_on_error = cpu_to_le32(static_cast<uint32_t>(o->continue_on_error));
...
int fio_test_cconv(struct thread_options *__o)
{
	...
	o1.verify_pattern = static_cast<char*>(malloc(o1.verify_pattern_bytes));
	memset(o1.verify_pattern, 'V', o1.verify_pattern_bytes);
	o1.buffer_pattern_bytes = 15;
	o1.buffer_pattern = static_cast<char*>(malloc(o1.buffer_pattern_bytes));
	memset(o1.buffer_pattern, 'B', o1.buffer_pattern_bytes);

	top_sz = thread_options_pack_size(&o1);
	top1 = static_cast<thread_options_pack*>(calloc(1, top_sz));
	top2 = static_cast<thread_options_pack*>(calloc(1, top_sz));
