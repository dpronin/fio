...
{
	return fio_crc32c(static_cast<unsigned char const*>(buf), len);
}
...
struct bloom *bloom_new(uint64_t entries)
{
	...
	b = static_cast<struct bloom *>(malloc(sizeof(*b)));
	b->nentries = entries;
	no_uints = (entries + BITS_PER_INDEX - 1) / BITS_PER_INDEX;
	b->map = static_cast<decltype(b->map)>(calloc(no_uints, sizeof(uint32_t)));
	if (!b->map) {
		free(b);
		return NULL;
	}
