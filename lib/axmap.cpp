...
struct axmap *axmap_new(uint64_t nr_bits)
{
	...
	axmap = static_cast<struct axmap *>(malloc(sizeof(*axmap)));
	if (!axmap)
		return NULL;
	...
	axmap->levels = static_cast<decltype(axmap->levels)>(calloc(axmap->nr_levels, sizeof(struct axmap_level)));
	if (!axmap->levels)
		goto free_axmap;
	...
	for (i = 0; i < axmap->nr_levels; i++) {
		...
		al->level = i;
		al->map_size = nr_bits;
		al->map = static_cast<decltype(al->map)>(malloc(al->map_size * sizeof(unsigned long)));
		if (!al->map)
			goto free_levels;
...
{
	struct axmap_set_data *data = static_cast<struct axmap_set_data *>(__data);
