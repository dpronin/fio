...
struct fifo *fifo_alloc(unsigned int size)
{
	...
	fifo = static_cast<struct fifo *>(malloc(sizeof(struct fifo)));
	if (!fifo)
		return NULL;

	fifo->buffer = static_cast<decltype(fifo->buffer)>(malloc(size));
	fifo->size = size;
	fifo->in = fifo->out = 0;
...
unsigned int fifo_put(struct fifo *fifo, void *buffer, unsigned int len)
{
	...
	memcpy(fifo->buffer, static_cast<char const*>(buffer) + l, len - l);
...
unsigned int fifo_get(struct fifo *fifo, void *buf, unsigned int len)
{
	...
	if (buf) {
		 ...
		 */
		memcpy(static_cast<char*>(buf) + l, fifo->buffer, len - l);
	}
