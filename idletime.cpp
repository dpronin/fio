...
static void *idle_prof_thread_fn(void *data)
{
	int retval;
	unsigned long j, k;
	struct idle_prof_thread *ipt = static_cast<struct idle_prof_thread *>(data);
...
void fio_idle_prof_init()
{
	...
	ipc.ipts = static_cast<decltype(ipc.ipts)>(malloc(ipc.nr_cpus * sizeof(struct idle_prof_thread)));
	if (!ipc.ipts) {
		log_err("fio: malloc failed\n");
		return;
	}

	ipc.buf = static_cast<decltype(ipc.buf)>(malloc(ipc.nr_cpus * page_size));
	if (!ipc.buf) {
		log_err("fio: malloc failed\n");
		free(ipc.ipts);
		return;
	...
	for (i = 0; i < ipc.nr_cpus; i++) {
		...
		ipt->data = static_cast<unsigned char*>(ipc.buf) + page_size * i;
